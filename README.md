...
- **Polls**: incoming federated polls (`Question` objects) show their options, counts and expiry in every timeline and can be voted on — votes federate to the poll's author as ActivityPub vote notes, authoritative counts come back as `Update{Question}` (`lib/Service/PollService.php`). Creating own polls is not supported. **No lists.**
