...
- 📦 **Migration** — a page of its own behind your account: **Export** takes your profile, follows, followers, blocks, mutes, bookmarks, likes and every post you have written as a zip (your private key is deliberately not in it), **Import** reads one back — including an archive from `occ user:export` — and a third section brings your follows over from Mastodon, Pixelfed, GoToSocial or Akkoma via their `following_accounts.csv`. **The pictures and videos come with the posts**: every file is copied into the archive under `media_attachments/`, in the layout Mastodon's own export uses, and each attachment's URL in `outbox.json` points at the copy rather than at the server you are leaving — so the archive is the posts and not a list of links that stop working. Your profile banner travels the same way, and an import puts the files back through the path an upload takes. It says plainly which networks cannot be imported from, and leaves a whole-account `Move` to `occ social:account:alias` and `occ social:account:move`, since that federates and cannot be undone.
...
- 😀 **Custom emoji** — `Emoji` tags on remote statuses and actors survive the cache (via the stored wire source) and are served in the `emojis` field of status and account entities; the web client shows them inline in content and display names. This instance publishes its own, too: an administrator adds one with `occ social:emoji add <shortcode> <file>` (PNG, GIF, WebP or JPEG, up to 256 KiB, metadata stripped like every other upload; `--hidden` keeps it usable by name but out of pickers, `--category` groups it), lists and removes them with the same command, and the picture is served from `/emoji/{shortcode}` for local clients and remote servers alike. `/api/v1/custom_emojis` lists the visible ones. Write `:shortcode:` in a post or its content warning and the post goes onto the wire with a matching `Emoji` tag, so it renders on instances that have never heard of it (`lib/Service/EmojiService.php`). The web composer's emoji picker offers Unicode emoji only — a custom one is typed by its shortcode.
...
- 📋 **Lists** — group the accounts you follow and read them as their own timeline (`/api/v1/lists`, `/api/v1/timelines/list/{id}`). The **Lists** section of the sidebar shows your lists and opens each one's timeline; creating a list and choosing its members is done through the API or a Mastodon client, since the web client has no list editor yet. A list is private to whoever made it.
...
- [docs/User-Guide.md](https://github.com/nextcloud/social/blob/master/docs/User-Guide.md)
