...
**Verified against:** app version 0.15.0, `master`, 2026-09-12. Every claim was
checked by reading the file it names.

**Work in flight.** Some of what a reader will want is in an open pull request
rather than on `master`, and this document says so where it matters: **#2110**
(scheduled statuses, an instance actor, HTML content with real links, a
`replies` collection, report forwarding, domain-block purging, per-inbox
delivery deduplication, `resolve=true`) and **#2111** (timeline interaction).
Anything not marked that way is true of `master` today.

This document supersedes the parity reviews written against 0.11.51 and 0.11.63,
and the "what can be improved" assessment written against 0.11.49: most of what
those listed is now either fixed or covered here, in
...
Social 0.15 is a capable, standards-correct ActivityPub server with a broad and
...
announcements with an admin UI, account notes, and a real admin API. Scheduled
statuses and their cron are in **#2110**; until that merges, `scheduled_at` is
parsed and ignored, and the post is published immediately.
...
**Working and correct on `master`:** signed delivery and verification including
RFC 9421, paged outbox, followers, following and featured collections,
`contentMap` and language, correct visibility addressing that fails closed on
unknown, `updated` on edits, inbox forwarding per ActivityPub 7.1.2 with the LD
signature preserved, and a retry window of 16 attempts on Sidekiq's `tries⁴+15`
backoff — about 49 hours, deliberately matching Mastodon.

**Added by #2110**, and worth knowing about because each is a thing a peer can
see: a dedicated instance actor with its own key pair signing outbound GETs
(`lib/Service/InstanceActorService.php`), a real `replies` collection,
Mastodon-shaped HTML content with `u-url mention` and `hashtag` anchors agreeing
with the `tag` array (`lib/Service/LinkifyService.php`), and per-inbox delivery
deduplication. Until it merges, outbound `content` is escaped plain text — every
link, mention and hashtag arrives on every peer as dead text — and a post
mentioning three people on one server is delivered to that server three times.
...
8. Graded domain blocks — it is block-outright or nothing, with no silence or
   limit tier and no `reject_media`. (#2110 makes a block remove what the
   instance already sent; it does not add the tiers.)
...
Credit where it is due. Of the nine defects listed at 0.11.63, seven are fixed
on `master`: local poll voting, the character limit, poll expiry, the
single-choice duplicate vote, `?remote=true`, and notification dismiss and clear
are all correct now, and notifications — called the largest functional gap in
that review — reach the Nextcloud bell. `scheduled_at` and three of the four
moderation findings (report forwarding, domain-block purging, and the delivery
deduplication that went with them) are in **#2110**.
...
| 6b | Merge #2110 | — | Scheduled posts, HTML content, `replies`, report forwarding, domain purge, an instance actor |
