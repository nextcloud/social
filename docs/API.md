...
| PATCH | `/api/v1/accounts/update_credentials` | public, no-csrf (viewer required, `write` scope) | JSON/form body; `note`, `locked`, `discoverable`, `indexable`, `source[privacy]`, `header` (multipart, the profile banner) and `fields_attributes` are supported | Sets the bio (`note`, at most 500 characters, stored as plain text and rendered to HTML on the way out — an absent `note` leaves the stored one alone rather than clearing it), whether new followers need manual approval (`manuallyApprovesFollowers`), whether the account may be listed in directories and indexed for search, and/or the profile metadata fields (at most four name/value pairs, both halves required; a list or an object keyed by index). `display_name` and `avatar` are **not** accepted: the name and the picture belong to the Nextcloud account and are changed where the account is. Other Mastodon profile fields are ignored. Returns the refreshed account entity. |
...
| POST | `/api/v1/admin/accounts/{id}/action` | public, no-csrf (**admin required**, `admin:write` scope) | `type` (`silence`, `suspend`, `none`), `text`, `report_id` | Applies the decision through `ModerationService`, which is the same call the admin panel makes: **silence** takes the account out of the public and global timelines and changes no data; **suspend** deletes what it has posted here, drops its cached actor and refuses what it sends afterwards. `none` lifts whatever stands — Mastodon reads `none` as "warn and take no action", but a warning is a strike in a history this app does not keep, so the only honest meaning left is the panel's own "lift". `text` is kept as the comment on a decision that stands. `sensitive` and `disable` are **422**: no state here corresponds to either. With `report_id` the report is resolved in the same call, so the decision and the report it came from cannot disagree. Answers `{}`. |
