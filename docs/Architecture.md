...
5. `ImportService::importFromJson()` parses the body into a typed object. A property that may be one value or a list (`attachment`, `tag`, `to`, `cc`) is read through `ACore::listOf()`, so a single object or string counts as a list of one, and in `to`/`cc` the short spellings of the public collection, `as:Public` and `Public`, are read as its full IRI; a document the models still cannot read — a `TypeError` inside one of them — is an `ActivityPubFormatException`, answered 400
