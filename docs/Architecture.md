...
| `social_feed_item` | An entry out of one of those feeds: its link, title, summary, thumbnail and date. Unique on (feed, guid) so a feed republished with one new entry does not repeat the other fifteen. Never federated and never a post — nothing here is attributed to anybody on this server, and nothing can be boosted or replied to |
