...
- **Collections:** followers, following and outbox answer with an `OrderedCollection` whose `first` and `last` point at `?page=N`, and with `?page=N` they answer with a real `OrderedCollectionPage` of `OrderedCollection::PAGE_SIZE` (40) items linked by `next`/`prev`, so a consumer can enumerate them. Followers and following list `type = Follow` rows only: the accepted `Loopback` row every actor has on itself, which the home timeline reads (`getHomeCollectionPrims()`), is not a follow. `featured` — the actor's pinned posts, which is how a remote server learns about a pin — is a single unpaged collection
