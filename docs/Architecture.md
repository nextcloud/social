...
| `social_feed_item` | An entry out of one of those feeds: its link, title, summary, thumbnail and date. Unique on (feed, guid) so a feed republished with one new entry does not repeat the other fifteen. Pruned after every read that added something, to the newest 500 per feed and nothing older than 180 days (`FeedsRequest::prune()`, on `social_feeditem_fp`). Never federated and never a post — nothing here is attributed to anybody on this server, and nothing can be boosted or replied to |
...
and, since 0.26.67, the trending section that used to sit above it as a caption
...
It asked `only_media` alone until 0.26.67, which is Mastodon's question — *does
