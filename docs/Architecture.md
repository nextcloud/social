...
A story is the one object here that is neither a post nor a collection. It is published to the author's followers as an `Add` whose object is a `Story` and withdrawn with a `Delete` — Pixelfed's verbs, because Pixelfed is the network that has stories — and it resolves at `/@{username}/stories/{id}` for a peer that would rather fetch it than trust the copy it was handed, under the same rule the client API keeps: the author, or a signed reader who follows them. `StoryInterface` takes the incoming side and keeps a story only when somebody here follows its author, for no longer than a day whatever the sender's `expiresAt` says. A `Create` is deliberately not used: it would put a story where posts go, and a server that does not know the type ignores an `Add` of one, which is the outcome to want.
